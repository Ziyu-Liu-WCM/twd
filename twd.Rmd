...
fit.df <- data.frame(design, tmp_y)
...
runtime <- system.time({ 
result <- CPLM(airway, design, n_threads = 5)
})


input_features<- as.data.frame(t(assay(airway)))
input_metadata<- data.frame(colData(airway)["dex"])

PBMC <- Tweedieverse(
  input_features,
  input_metadata, 
  output = 'PBMC', # Assuming demo_output exists
  fixed_effects = c('dex'),
  base_model = 'CPLM',
  adjust_offset = TRUE)
```
